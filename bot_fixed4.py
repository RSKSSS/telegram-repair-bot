...
# Вспомогательные функции, которые отсутствуют в исходном коде
...
def updated_format_orders_list(orders: List[Dict[str, Any]], include_details: bool = True) -> str:
    """
    Форматирует список заказов для отображения
    
    Args:
        orders: Список заказов
        include_details: Включать ли детали заказа
        
    Returns:
        Отформатированный текст со списком заказов
    """
    if not orders:
